...
- Admin UI уже поддерживает persisted `RU`/`EN` language switch и несколько additive operator follow-up surfaces: recent UI tasks, pipeline run detail picker, pipeline stage filter, matrix list/detail filters, quick-pick review controls для queue/history/submit flows, compact Tasks-page controls для quick-pick/filter/search/sort по recent tasks, explicit multilingual hardening для dashboard, outputs, KB и documents controls/detail actions, а также localized table headers across dashboard, documents, pipeline, matrix, review, scoring, outputs, KB и tasks surfaces.
...
- Основной разрыв сместился в operator productivity и day-2 usability: быстрый follow-up для async workflows, cross-surface linkage, компактная observability и остаточные detail-renderer / helper-level display-layer polishing gaps все еще расширяются итеративно.
...
| P0 | Multilingual operator polish | Display-layer labels, detail actions и empty states доводятся до согласованного `RU`/`EN` поведения без изменения internal keys и payload semantics. |
| P0 | Detail renderer polish | Detail summaries, compact pickers и remaining table-like detail blocks выравниваются по display semantics и translation coverage без затрагивания API contracts. |
| P0 | Helper regression expansion | Pure-function and helper-level regression batches покрывают operator-hardening slices и снижают риск тихих regressions в additive UI logic. |
| P0 | Cross-surface handoff | Operator быстрее открывает связанные runs, tasks, outputs, artifacts и entities из ближайшего контекста без ручного копирования id. |
...
## 10.4 Актуализированный execution plan

1. Синхронизировать canon docs с фактическим head: stage, validated baseline, active tranche, immediate priorities.
2. Дозакрывать remaining detail-renderer и compact-summary display gaps small-slice правками без изменения API contracts.
3. Наращивать helper-level regression density вокруг filter/sort/resolve/format logic и i18n-sensitive pure UI functions.
4. Продолжать additive cross-surface handoff между pipeline, tasks, outputs, KB и review там, где оператор еще переносит id вручную.
5. Обновлять release evidence только после реального rerun, а не после doc-only/operator-polish sync.
...
7. Документ отражает, что текущий operator-hardening head уже включает multilingual display-layer polish для dashboard, documents, outputs, KB, review, scoring, tasks и связанных detail/actions surfaces.
8. Документ фиксирует, что текущий head уже локализует ключевые tabular/operator summaries, включая table headers на dashboard, documents, pipeline, matrix, reviews, scoring overview/diff, outputs, KB и tasks surfaces.
9. Документ фиксирует helper-level regression expansion и cross-surface handoff как ближайший P0 execution tranche поверх уже достигнутого release-ready MVP.