#!/usr/bin/env python3
"""CLI checker for governance score."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Check quality gate governance score")
...
def _fetch_report(*, api_base: str, params: dict[str, Any], timeout: float) -> dict[str, Any]:
    response = httpx.get(
        f"{api_base.rstrip('/')}/outputs/quality-gate/governance-score",
        params=params,
        timeout=timeout,
    )
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("governance-score endpoint returned non-object payload")
    return payload
...
def run(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"governance-score check: API request failed: {exc}", file=sys.stderr)
        return 2
    ...
    print(f"governance-score check: score={score:.2f} status={status}")

    if score < args.min_score:
        print(
            f"governance-score check: FAILED (score {score:.2f} below min {args.min_score:.2f})",
            file=sys.stderr,
        )
        return 1

    print("governance-score check: OK")
    return 0
