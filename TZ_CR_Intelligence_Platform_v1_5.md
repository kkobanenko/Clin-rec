...
- Additive operator follow-up уже расширен recent UI task tracking, pipeline run detail picker, pipeline stage filter, matrix list/detail filters, review quick-pick controls, compact Tasks-page quick-pick/filter/search/sort controls и explicit multilingual hardening для dashboard/documents/outputs/KB detail-action and filter surfaces без изменения compatibility contract.
...
- Не все operator workflows одинаково удобны для day-2 эксплуатации: остаются локальные разрывы в async follow-up, linkage, compact observability и остаточном display-layer polish на отдельных admin surfaces.
...
- Review, matrix, outputs, KB, documents и Tasks surfaces продолжают получать compact quick-pick/filter/search controls и explicit translation hardening, сокращающие ручной перенос id между operator flows и display-layer inconsistency.

**Статус:** в процессе; текущий head уже покрывает task/run follow-up, matrix/review/tasks quick-picks и серию multilingual/operator-polish micro-slices.
...
**Статус:** в процессе; compact filters и detail follow-up уже расширены на pipeline, matrix, outputs и KB, но единый linkage-view все еще остается post-current tranche шагом.
