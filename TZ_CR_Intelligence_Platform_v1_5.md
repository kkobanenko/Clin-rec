...
- Additive operator follow-up уже расширен recent UI task tracking, pipeline run detail picker, pipeline stage filter, matrix list/detail filters, review quick-pick controls, compact Tasks-page quick-pick/filter/search/sort controls, explicit multilingual hardening для dashboard/documents/outputs/KB detail-action and filter surfaces и localized table headers across dashboard/documents/pipeline/matrix/reviews/scoring/outputs/KB/tasks без изменения compatibility contract.
...
- Не все operator workflows одинаково удобны для day-2 эксплуатации: остаются локальные разрывы в async follow-up, linkage, compact observability и остаточном detail-renderer/helper-level display-layer polish на отдельных admin surfaces.
...
- Regression density по helper-level UI logic и additive operator glue остается неравномерной: часть pure helpers покрыта хорошо, часть recent multilingual/operator-polish paths все еще опирается на более узкие point tests и live smoke.
...
| Operator hardening | Recent task tracking, run-detail follow-up, compact filters, multilingual display-layer polish и другие additive UI/API follow-up surfaces |
...
- Review, matrix, outputs, KB, documents и Tasks surfaces продолжают получать compact quick-pick/filter/search controls, explicit translation hardening и localized operator summaries/table headers, сокращающие ручной перенос id между operator flows и display-layer inconsistency.

**Статус:** в процессе; текущий head уже покрывает task/run follow-up, matrix/review/tasks quick-picks, localized operator table headers и серию multilingual/operator-polish micro-slices.
...
**Статус:** в процессе; compact filters и detail follow-up уже расширены на pipeline, matrix, outputs и KB, но remaining detail-renderer polish и единый linkage-view все еще остаются следующим post-current tranche шагом.
...
**Статус:** в процессе; базовый snapshot уже существует, richer release cockpit остается следующим, а не blocking шагом.

## 6.10 Task J — Helper-level regression expansion
...
| Зависимости | Task G |
| Оценка | 0.5-1 день |
| Результат | Recent additive helper logic и multilingual display paths получают плотный regression слой без full UI E2E на каждый micro-slice |

**Технические изменения:**

- Расширяются batch-style tests для pure helpers из `app/ui/app.py` и связанных lightweight i18n helpers.
- Проверяются filter/sort/resolve/format functions, frontmatter parsing, quick-pick helpers и remaining display-sensitive pure functions.
- Validation остается focused: targeted pytest suites для helper slices, затем live smoke только для change sets с UI-visible impact.

**Статус:** ближайший P0 tranche.

## 6.11 Task K — Canonical stage and roadmap sync
...
| Зависимости | Нет |
| Оценка | 0.25 дня |
| Результат | PRD/TZ/README/VERSIONING/backlog отражают текущую стадию и следующий execution sequence без расхождения с validated evidence |

**Технические изменения:**

- Документы синхронизируются с реальным состоянием head: `release-ready MVP`, active `phase-2 operator hardening`, validated full-pack baseline и ближайшие small-slice priorities.
- Release summary не переписывается без фактического rerun; doc sync не выдает старую evidence baseline за новую.
- Следующий tranche формулируется как helper regression + detail/linkage polish, а не как новая базовая backend phase.

**Статус:** ongoing discipline для каждого заметного operator tranche.
...
| `tests/test_ui_regression_batches.py` | Batch-style regression harness для pure UI helpers и additive operator glue |
