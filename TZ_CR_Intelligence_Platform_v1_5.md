...
- Зафиксированы downstream integration checks для evidence/matrix и KB/output workflows.
...
| Оценка | 1-2 дня |
| Результат | Downstream evidence/matrix и KB/output workflows проверены как часть release process |
...
| `app/ui/app.py` | Admin/operator UI path для release-critical actions |
...
3. Smoke summary и API results должны быть совместимы по терминологии и ожиданиям.
...
| Integration | KB integration и downstream evidence/matrix verification |
