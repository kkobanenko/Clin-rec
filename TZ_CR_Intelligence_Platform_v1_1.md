...
> **Superseded:** актуальное ТЗ — [TZ_CR_Intelligence_Platform_v1_2_kb.md](TZ_CR_Intelligence_Platform_v1_2_kb.md) (v1.3 в шапке файла).
...
| Sync DB driver | Для sync SQLAlchemy engine в worker обязателен `psycopg2-binary` (или совместимый эквивалент). |
...
- Таблица ниже фиксирует MVP-минимум и не является исчерпывающим перечнем endpoint-ов платформы.
...
## 15.1 Профили развертывания и маршрутизация очередей

- Допускаются два профиля запуска: host-only (API + worker на хосте) и docker-compose-only (API + worker в одном compose проекте).

- В пределах одного запуска запрещено смешивать профили так, чтобы API и worker были подключены к разным Redis endpoints.

- Для каждого запуска значения `CRIN_CELERY_BROKER_URL` и `CRIN_CELERY_RESULT_BACKEND` у API и worker должны совпадать.

- Run со статусом `pending` не должен считаться терминальным; polling обязан продолжаться до `completed` или `failed`.
...
- Для run со статусом `completed` `stats_json` обязан содержать минимум полей: `discovery_service_version`, `run_type`, `wall_time_seconds`, `total_discovered`, `duplicates_detected`, `coverage_percent`.
...
| E2E smoke | Проверка `/sync/full` → `/runs/{id}` → `/documents` с допуском статуса `pending` в процессе polling и `stats_json = null` до завершения run. |
...
8. Runtime profile: API и worker подключены к одному broker/backend и подтверждённо обрабатывают discovery-задачи в выбранном профиле запуска.

9. E2E smoke script: корректно обрабатывает промежуточные статусы (`pending`, `running`) и отсутствие `stats_json` до завершения run.
...
- Все операции с docker-контейнерами и портами выполняются только в рамках изоляции проекта согласно [ISOLATION_POLICY.md](ISOLATION_POLICY.md).
...
1. Канонический DoD определяется в [DOD_MVP.md](DOD_MVP.md).

2. TZ использует этот DoD как единственный источник критериев без дублирования конфликтующих списков.
